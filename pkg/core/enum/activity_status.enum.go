...
func AstatsFromStrings(in []string) ([]ActivityStatus, error) {
	out := make([]ActivityStatus, 0, len(in))

	for _, v := range in {
		var astat ActivityStatus
		if err := astat.Scan(v); err != nil {
			return nil, err
		}
		out = append(out, astat)
	}
